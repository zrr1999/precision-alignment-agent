...
本目录包含 **人工维护的 Paddle 相关知识**，供 Paddle Pilot 系统的各个智能体**只读访问**。
...
**重要**: 智能体不得修改此目录下的任何内容。自动生成的知识应存放在 `.paddle-pilot/memory/` 目录下。
...
| 特性 | `knowledge/` (人工) | `.paddle-pilot/memory/` (自动) |
|------|---------------------|----------------------|
...
2. **然后查询** `.paddle-pilot/memory/` - 获取历史任务的具体经验
3. **综合分析** - 结合两者制定策略
