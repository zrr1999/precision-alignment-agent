...
- **Session**: `session_id` from caller. If missing, question the caller; use placeholder path that caller can correct.
...
2. **PR list**: Related **Paddle** PRs found (title, URL, merged/closed, relevance score 1–5). Prefer: precision alignment, accuracy fix, dtype promotion, numerical consistency in Paddle.
...
- **Limit**: 5–10 most relevant **Paddle** PRs; 3–5 deep dives for fix extraction.

## Session report (short-term memory)

- **End (write session-level report)**: Write this run's conclusions to `.paa/sessions/{session_id}/learner/{api_name}/prior-art.md`.
  - `session_id` from caller; use for all report paths.
