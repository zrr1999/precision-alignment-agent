...
- **Commit**: `git add` and `git commit` with message starting `[PAA]` after build + smoke pass; then append a progress entry (commit hash, message, cases fixed) to `.paa/sessions/{api_name}/progress.md` and output that summary to the caller.
...
1. **Learner** (optional, for prior art): Spawn with `api_name` to find and study related precision-alignment PRs. Use the prior-art report to inform roadmap and fix strategy.
...
8. **Session report**: Write this run's conclusions to `.paa/sessions/{api_name}/{short-title}.md`. Reference or summarize the per-commit progress from `progress.md`. If you discover cross-API reusable knowledge, call `paa-knowledge-curation` to append to `.paa/memory/{topic}.md`.

## Sub-Agent Rules

- **Learner**: Optional; spawn with `api_name` to gather prior art; use report in roadmap.
...
- **Per-commit progress (during AD loop)**: After each successful commit, append to `.paa/sessions/{api_name}/progress.md` one entry (commit hash, commit message, cases fixed in this commit). Output the same summary to the caller. Reviewer will merge this file when squashing commits.
- **End (write session-level report only)**:
  - Write this task’s overall precision-comparison conclusions, key decisions, trade-offs, and remaining gaps to `.paa/sessions/{api_name}/{short-title}.md`.
...
- No edit/write: code changes by Aligner only. No direct code analysis: use Explorer reports. Bash: git only. Treat `knowledge/commons/` and `.paa/memory/` as read-only; write only to `.paa/sessions/{api_name}/...` for this task, or to `.paa/memory/` via the knowledge-curation skill when abstracting long-term patterns.
